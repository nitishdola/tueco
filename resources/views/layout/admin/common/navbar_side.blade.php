<li>
    <a  href="index.html"><i class="fa fa-dashboard"></i> Dashboard</a>
</li> 
<li>
    <a href="#" class="active-menu"><i class="fa fa-line-chart"></i> Accounting<span class="fa arrow"></span></a>
    <ul class="nav nav-second-level collapse in">
        <li><a href="#"><i class="fa fa-plus-circle"></i> Masters<span class="fa arrow"></span></a>
            <ul class="nav nav-third-level">
                <li>
                    <a href="chart.html">Accounting Heads</a>
                </li>
                <li>
                    <a href="morris-chart.html">Ledgers</a>
                </li>
                <li>
                    <a href="morris-chart.html">Members</a>
                </li>
            </ul>
        </li>	 
        <li>
            <a href="#"><i class="fa fa-indent"></i> Vouchers <span class="fa arrow"></a>
            <ul class="nav nav-third-level">
            <li>
                    <a href="chart.html">Payment Voucher</a>
                </li>
                <li>
                    <a href="morris-chart.html">Receipt Voucher</a>
                </li>
                <li>
                    <a href="morris-chart.html">Journal Voucher</a>
                </li>
                <li>
                    <a href="morris-chart.html">Contra Voucher</a>
                </li>
                </ul>
        </li>  
        <li>
            <a href="empty.html"><i class="fa fa-file-text"></i> Reports<span class="fa arrow"></a>
            <ul class="nav nav-third-level">
            <li>
                    <a href="chart.html">Payment and Receipt</a>
                </li>
                <li>
                    <a href="morris-chart.html">Daily Cash Trail</a>
                </li>
                <li>
                    <a href="morris-chart.html">Ledger Book</a>
                </li>
                <li>
                    <a href="morris-chart.html">Cash Book</a>
                </li>
                <li>
                    <a href="morris-chart.html">Income and Expenditure</a>
                </li>
                <li>
                    <a href="chart.html">Test Report 1</a>
                </li>
                <li>
                    <a href="morris-chart.html">Test Report 2</a>
                </li>
                <li>
                    <a href="morris-chart.html">Test Report 3</a>
                </li>
                <li>
                    <a href="morris-chart.html">Test Report 4</a>
                </li>
                </ul>
        </li>
    </ul>
</li> 
<li>
    <a href="empty.html"><i class="fa fa-dollar"></i> Loan Management System<span class="fa arrow"></a>
    <ul class="nav nav-second-level">
    <li>
            <a href="chart.html">Payment and Receipt</a>
        </li>
        <li>
            <a href="morris-chart.html">Daily Cash Trail</a>
        </li>
        <li>
            <a href="morris-chart.html">Ledger Book</a>
        </li>
        <li>
            <a href="morris-chart.html">Cash Book</a>
        </li>
        <li>
            <a href="morris-chart.html">Income and Expenditure</a>
        </li>
		</ul>
</li>